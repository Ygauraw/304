...
class NormalSolitaire extends Rules {
	...
	public void Init(Bundle map) {
		...
		mCardAnchor[0] = CardAnchor.CreateAnchor(CardAnchor.SEQ_SINK, 0, this);
		mCardAnchor[1] = CardAnchor.CreateAnchor(CardAnchor.SEQ_SINK, 1, this);
		mCardAnchor[2] = CardAnchor.CreateAnchor(CardAnchor.SEQ_SINK, 2, this);
		if (mDealThree) {
	...
	public void Resize(int width, int height) {
		...
		mCardAnchor[7].SetPosition(rem + 1 * (rem + Card.WIDTH), 30 + 2 * Card.HEIGHT);
	...
	private boolean TryToSinkCard(CardAnchor anchor, Card card) {
		for (int i = 0; i < 4; i++) {
			if (mCardAnchor[i + 2].DropSingleCard(card)) {
				mMoveHistory.push(new Move(anchor.GetNumber(), i + 2, 1, false, anchor
						.UnhideTopCard()));
			...
			if (anchor.GetCount() - anchor.GetHiddenCount() >= 13) {
				...
				if (card[anchor.GetCount() - 1].GetValue() == 1) {
					...
					if (val == 14) {
						...
						mMoveHistory.push(new Move(anchor.GetNumber(), 11, 13, true, anchor
								.UnhideTopCard()));
